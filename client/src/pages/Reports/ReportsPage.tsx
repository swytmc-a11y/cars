...
import { TrendingUp, Car, FileText, DollarSign, FileDown, Loader2, ChevronDown } from "lucide-react";
...
export default function ReportsPage() {
  ...
  const pdfOptions = {
    onSuccess: ({ url }: { url: string }) => window.open(url, "_blank"),
    onError: (e: { message: string }) => toast.error(e.message),
    onSettled: () => setPendingPdf(null),
  };
  const fleetPdfMutation = trpc.reports.fleetPdf.useMutation(pdfOptions);
  const revenuePdfMutation = trpc.reports.revenuePdf.useMutation(pdfOptions);
  const contractsPdfMutation = trpc.reports.contractsPdf.useMutation(pdfOptions);
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" disabled={pendingPdf !== null}>
                {pendingPdf ? <Loader2 className="h-4 w-4 ml-2 animate-spin" /> : <FileDown className="h-4 w-4 ml-2" />}
                {pendingPdf ? "جاري إنشاء التقرير..." : "تقرير PDF"}
                <ChevronDown className="h-3.5 w-3.5 mr-1" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start">
              <DropdownMenuItem onClick={() => { setPendingPdf("fleet"); fleetPdfMutation.mutate(); }}>
                تقرير حالة الأسطول
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => { setPendingPdf("revenue"); revenuePdfMutation.mutate({ months: 6 }); }}>
                تقرير الإيرادات الشهرية
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => { setPendingPdf("contracts"); contractsPdfMutation.mutate({}); }}>
                تقرير العقود
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
